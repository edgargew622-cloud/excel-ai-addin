  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        destSheet: { type: "string", description: "Существующий лист назначения. Пусто — тот же лист, что и источник." },
        rows: {
          type: "array",
          minItems: 1,
...
- Сводную строй через create_pivot_table по всей области с шапкой. Для текстовых полей в values используй count: сумма текста — ноль. Если место занято, не строй поверх — предложи другое место или отдельный лист. Итоги в отчёте бери из grandTotals ответа; предупреждения из warnings перескажи.
