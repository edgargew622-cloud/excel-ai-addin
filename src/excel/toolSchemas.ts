...
- Новый лист создаётся через create_sheet: он пуст, данные и другие листы не трогаются. Создавай его, когда пользователь просит отдельный лист или когда результат некуда положить, — но не по своей инициативе ради «чистого места». Переименовывать, копировать и удалять листы агент пока не умеет; об этом говори прямо.
