...
- Фильтр на ту же область добавляет условие к уже стоящим, а фильтр на другую область заменяет прежний целиком. Что произошло, бери из filterChange ответа — new, adds, replacesColumn или replacesFilter — и не утверждай, что фильтр заменён, если это не так. В отчёте называй visibleRowsBefore и visibleRowsAfter из ответа.
